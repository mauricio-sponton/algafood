...
public interface GrupoPermissaoControllerOpenApi {
	...
	CollectionModel<PermissaoDTO> listar(@ApiParam(value = "ID de um grupo", required = true) Long grupoId);
	...
	ResponseEntity<Void> desassociar(@ApiParam(value = "ID de um grupo", required = true) Long grupoId, @ApiParam(value = "ID de uma permissão", required = true) Long permissaoId);
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "404", description = "Grupo ou permissão não encontrado", content = @Content(schema = @Schema(implementation = Problema.class))) })
	ResponseEntity<Void> associar(@ApiParam(value = "ID de um grupo", required = true) Long grupoId, @ApiParam(value = "ID de uma permissão", required = true)Long permissaoId);
