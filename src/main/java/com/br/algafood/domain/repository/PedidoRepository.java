...
public interface PedidoRepository extends JpaRepository<Pedido, Long>, JpaSpecificationExecutor<Pedido>{
	...
	@Query("select case when count(1) > 0 then true else false from Pedido p join p.restaurante rest join rest.responsaveis resp where p.codigo = :codigoPedido and resp.id = : usuarioId")
